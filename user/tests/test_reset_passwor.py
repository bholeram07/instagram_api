...
class ResetPasswordTestCase(APITestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            email="testuser@example.com", password="oldpassword123", username="abcd"
        )
        self.token = PasswordResetTokenGenerator().make_token(self.user)
        self.user_id = urlsafe_base64_encode(str(self.user.id).encode())

    def test_reset_password_success(self):
        data = {"new_password": "Bhole057p@", "confirm_password": "Bhole057p@"}
        context = {"user_id": self.user_id, "token": self.token}
    ...
    def test_reset_password_password_mismatch(self):
        ...
        self.assertIn(
            "password does not match with the confirm password", str(serializer.errors)
        )
    ...
    def test_reset_password_invalid_token(self):
        ...
        context = {"user_id": self.user_id, "token": invalid_token}
